...
public class UdpClient
{
    ...
    public void Connect(string host, int port)
    {
        ...
        UnityEngine.Debug.LogWarning("==udp完成连接："+ host+","+ port);
    }
            ...
            try
            {
                ...
                else if (index == 1)
                {
                    ...
                    if (result > 0)
                    {
                        if (result <20)
                        {
                            UnityEngine.Debug.LogError("奇怪的数据：" + result);
                        }
                        else
                        {
    ...
    public void Send(UInt32 id, UInt32 protocol, Int32 cmd, byte[] msg)
    {
        ...
        try
        {
            ...
            UnityEngine.Debug.LogWarning("==发送Udp验证数据==");
        }
        catch
        {
        }
    ...
    public int Send(byte[] datas)
    {
        ...
        {
            UnityEngine.Debug.Log("发送错误：" + ex.ToString());
        }
        return 0;
    }
    #endregion
